...
# Need Updation
    def cleanWithVertex(self, edges_mask, groups):
        edges_mask = edges_mask.astype(bool)
        ...
        new_indices = np.zeros(edges_mask.shape[0], dtype=np.int32)
        new_indices[-1] = -1
        new_indices[edges_mask] = np.arange(0, np.where(edges_mask)[0].shape[0])
        self.gemm_edges[:, :] = new_indices[self.gemm_edges[:, :]]
        ...
        new_vs_indices[-1] = -1
        new_vs_indices[v_mask] = np.arange(0, np.where(v_mask)[0].shape[0])
    ...
    def init_history(self):	
        ...
        elif self.opt.feat_from == 'vertex':
            self.history_data = {
                'groups': [],
                'gemm_faces': [self.gemm_faces.copy()],
                'gemm_edges': [self.gemm_edges.copy()],
                ...
                'sides': [self.sides.copy()],
                'face_areas': [self.face_areas.copy()],
                've': [self.ve.copy()]  # Add the 've' attribute for vertex feature
            }
    ...
    def __clean_history(self, groups, pool_mask):
        if self.history_data is not None:
            ...
            if self.opt.feat_from == 'edge':	
                ...
                self.history_data['current2old'][0: self.vs_count] = np.where(mask)[0]
                self.history_data['gemm_faces'].append(self.gemm_faces.copy())
                self.history_data['face_count'].append(self.face_count)
                self.history_data['vs'].append(self.vs.copy())
                self.history_data['vs_count'].append(self.vs_count)
                self.history_data['ve'].append(self.ve.copy())  # Add the 've' attribute for vertex feature
    ...
    def unroll_gemm(self):
        ...
        if self.opt.feat_from == 'face' or self.opt.feat_from == 'vertex':
            ...
            if self.opt.feat_from == 'vertex':
                ...
                self.history_data['ve'].pop()  # Remove vertex-edge adjacency data
