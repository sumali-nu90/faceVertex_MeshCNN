...
class VertexMeshPool(nn.Module):
    ...
    def __pool_main(self, mesh_index):
        ...
        mesh.cleanWithVertex(edge_mask, vertex_groups)
    ...
    def __pool_edge(self, mesh, edge_id, mask, vertex_groups):
        if self.has_boundaries(mesh, edge_id):
            ...
            mask[edge_id] = False
            MeshPoolVertex.__union_groups(mesh, vertex_groups, removed_v, updated_v)
            MeshPoolVertex.__remove_group(mesh, vertex_groups, removed_v)
            mesh.edges_count -= 1
            return True
        else:
            return False
    ...
    def __clean_side(self, mesh, edge_id, mask, vertex_groups, side):
        if mesh.edges_count <= self.__out_target:
            return False
        invalid_edges = MeshPoolVertex.__get_invalids(mesh, edge_id, vertex_groups, side)
    ...
    def __pool_side(self, mesh, edge_id, mask, vertex_groups, side):
        info = MeshPoolVertex.__get_face_info(mesh, edge_id, side)
    ...
    def __get_invalids(mesh, edge_id, vertex_groups, side):
        ...
        shared_items = MeshPoolVertex.__get_shared_items(other_keys_a, other_keys_b)
        if len(shared_items) == 0:
            ...
            MeshPoolVertex.__redirect_edges(mesh, edge_id, side, update_key_a, update_side_a)
            MeshPoolVertex.__redirect_edges(mesh, edge_id, side + 1, update_key_b, update_side_b)
            MeshPoolVertex.__redirect_edges(mesh, update_key_a, MeshPoolVertex.__get_other_side(update_side_a),
                                           update_key_b, MeshPoolVertex.__get_other_side(update_side_b))
    ...
    def __remove_triplete(self, mesh, edge_mask, vertex_groups, invalid_edges):
        ...
        for neighbor in vertices:
            MeshPoolVertex.__union_groups(mesh, vertex_groups, vertex[0], neighbor)
        MeshPoolVertex.__remove_group(mesh, vertex_groups, vertex[0])
