...
from models.layers.face_mesh_conv import FaceMeshConv
from models.layers.face_mesh_pool import FaceMeshPool
from models.layers.face_mesh_unpool import FaceMeshUnpool
...
def define_classifier(input_nc, ncf, ninput_features, nclasses, opt, gpu_ids, arch, init_type, init_gain, feat_from,
    ...
    if arch == 'mconvnet':
        ...
        elif feat_from == 'vertex':  #Need Changes
            net = MeshConvNetVertex(norm_layer, input_nc, ncf, nclasses, ninput_features, opt.pool_res, opt.fc_n,
                                    opt.resblocks, symm_oper=opt.symm_oper)
        ...
        if feat_from == 'edge':
            ...
            net = MeshEncoderDecoderVertex(pool_res,
                                        down_convs,
                                        up_convs,
                                        blocks=opt.resblocks,
                                        transfer_data=True,
...
class MeshConvNetFace(nn.Module):
    ...
    def __init__(self, norm_layer, nf0, conv_res, nclasses, input_res, pool_res, fc_n,
        ...
        for i, ki in enumerate(self.k[:-1]):
            ...
            setattr(self, 'norm{}'.format(i), norm_layer(**norm_args[i]))
            setattr(self, 'pool{}'.format(i), FaceMeshPool(self.res[i + 1]))
...
class MResConvFace(nn.Module):
    def __init__(self, in_channels, out_channels, skips=1, symm_oper=None):
        ...
        self.out_channels = out_channels
        self.skips = skips
        self.conv0 = FaceMeshConv(self.in_channels, self.out_channels, bias=False)
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm2d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
                    FaceMeshConv(self.out_channels, self.out_channels, bias=False))
...
class DownConvFace(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, pool=0, symm_oper=None):
        ...
        self.bn = []
        self.pool = None
        self.conv1 = FaceMeshConv(in_channels, out_channels)
        self.conv2 = []
        for _ in range(blocks):
            self.conv2.append(FaceMeshConv(out_channels, out_channels))
            self.conv2 = nn.ModuleList(self.conv2)
        for _ in range(blocks + 1):
            self.bn.append(nn.InstanceNorm2d(out_channels))
            self.bn = nn.ModuleList(self.bn)
        if pool:
            self.pool = FaceMeshPool(pool)
...
class UpConvFace(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, unroll=0, residual=True,
        ...
        self.unroll = None
        self.transfer_data = transfer_data
        self.up_conv = FaceMeshConv(in_channels, out_channels)
        self.relu = relu
        if transfer_data:
            self.conv1 = FaceMeshConv(2 * out_channels, out_channels)
        else:
            self.conv1 = FaceMeshConv(out_channels, out_channels)
        self.conv2 = []
        for _ in range(blocks):
            self.conv2.append(FaceMeshConv(out_channels, out_channels))
            self.conv2 = nn.ModuleList(self.conv2)
        if batch_norm:
            ...
            self.bn = nn.ModuleList(self.bn)
        if unroll:
            self.unroll = FaceMeshUnpool(unroll)
...
# Need Modification
class MeshConvNetVertex(nn.Module):
    def __init__(self, norm_layer, nf0, conv_res, nclasses, input_res, pool_res, fc_n,
                 nresblocks=3, symm_oper=None):
        super(MeshConvNetVertex, self).__init__()
        self.k = [nf0] + conv_res
        self.res = [input_res] + pool_res
        norm_args = get_norm_args(norm_layer, self.k[1:])

        for i, ki in enumerate(self.k[:-1]):
            setattr(self, 'conv{}'.format(i), MResConvVertex(ki, self.k[i + 1], nresblocks))
            setattr(self, 'norm{}'.format(i), norm_layer(**norm_args[i]))
...
class MResConvVertex(nn.Module):
    def __init__(self, in_channels, out_channels, skips=1, symm_oper=None):
        super(MResConvVertex, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.skips = skips
        self.conv0 = VertexMeshConv(self.in_channels, self.out_channels, bias=False)
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm1d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
...
class MeshEncoderDecoderVertex(nn.Module):
    """Network for fully-convolutional tasks (segmentation) using vertex-based features
    """

    def __init__(self, pools, down_convs, up_convs, blocks=0, transfer_data=True, symm_oper=None):
        super(MeshEncoderDecoderVertex, self).__init__()
        self.transfer_data = transfer_data
        self.encoder = MeshEncoderVertex(pools, down_convs, blocks=blocks)
        unrolls = pools[:-1].copy()
        unrolls.reverse()
        self.decoder = MeshDecoderVertex(unrolls, up_convs, blocks=blocks, transfer_data=transfer_data,
                                         symm_oper=symm_oper)
...
class MeshEncoderVertex(nn.Module):
    def __init__(self, pools, convs, fcs=None, blocks=0, global_pool=None, symm_oper=None):
        super(MeshEncoderVertex, self).__init__()
        self.fcs = None
        self.convs = []
        for i in range(len(convs) - 1):
            if i + 1 < len(pools):
                ...
                pool = 0
            self.convs.append(DownConvVertex(convs[i], convs[i + 1], blocks=blocks, pool=pool))
        self.global_pool = None
        if fcs is not None:
            self.fcs = []
            self.fcs_bn = []
...
class MeshDecoderVertex(nn.Module):
    def __init__(self, unrolls, convs, blocks=0, batch_norm=True, transfer_data=True, symm_oper=None):
        super(MeshDecoderVertex, self).__init__()
        self.up_convs = []
        for i in range(len(convs) - 2):
            if i < len(unrolls):
                unroll = unrolls[i]
                ...
                unroll = 0
            self.up_convs.append(UpConvVertex(convs[i], convs[i + 1], blocks=blocks, unroll=unroll,
                                              batch_norm=batch_norm, transfer_data=transfer_data))
        self.final_conv = UpConvVertex(convs[-2], convs[-1], blocks=blocks, unroll=False,
                                       batch_norm=batch_norm, transfer_data=False)
        self.up_convs = nn.ModuleList(self.up_convs)
        reset_params(self)
...
class DownConvVertex(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, pool=0, symm_oper=None):
        super(DownConvVertex, self).__init__()
        self.bn = []
        self.pool = None
...
class UpConvVertex(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, unroll=0, residual=True,
                 batch_norm=True, transfer_data=True, symm_oper=None, relu=True):
        super(UpConvVertex, self).__init__()
        self.residual = residual
        self.bn = []
        self.unroll = None
        self.transfer_data = transfer_data
